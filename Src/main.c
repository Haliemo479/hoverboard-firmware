...
extern TIM_HandleTypeDef htim_front;
extern TIM_HandleTypeDef htim_rear;
...
extern P    rtP_Front;                   /* Block parameters (auto storage) */
extern P    rtP_Rear;                  /* Block parameters (auto storage) */
extern ExtY rtY_Front;                   /* External outputs */
extern ExtY rtY_Rear;                  /* External outputs */
extern ExtU rtU_Front;                   /* External inputs */
extern ExtU rtU_Rear;                  /* External inputs */
//---------------
...
extern volatile int pwml;               // global variable for pwm front. -1000 to 1000
extern volatile int pwmr;               // global variable for pwm rear. -1000 to 1000
...
extern SerialSideboard Sideboard_F;
#endif
#if defined(SIDEBOARD_SERIAL_USART3)
extern SerialSideboard Sideboard_R;
#endif
#if (defined(CONTROL_PPM_FRONT) && defined(DEBUG_SERIAL_USART3)) || (defined(CONTROL_PPM_REAR) && defined(DEBUG_SERIAL_USART2))
extern volatile uint16_t ppm_captured_value[PPM_NUM_CHANNELS+1];
#endif
#if (defined(CONTROL_PWM_FRONT) && defined(DEBUG_SERIAL_USART3)) || (defined(CONTROL_PWM_REAR) && defined(DEBUG_SERIAL_USART2))
...
int16_t front_dc_curr;            // global variable for Front DC Link current
int16_t rear_dc_curr;           // global variable for Rear DC Link current
int16_t dc_curr;                 // global variable for Total DC Link current
int16_t cmdF;                    // global variable for Front Command
int16_t cmdR;                    // global variable for Rear Command
...
static uint8_t Sideboard_Leds_F;
#endif
...
int main(void) {
  ...
  int16_t board_temp_adcFilt  = adc_buffer.temp;
    ...
    if (adc_buffer.l_tx2 > input1[0].min + 50 && adc_buffer.l_rx2 > input2[0].min + 50) {
      ...
      rtP_Front.n_max = rtP_Rear.n_max = MULTI_MODE_M3_N_MOT_MAX << 4;
      rtP_Front.i_max = rtP_Rear.i_max = (MULTI_MODE_M3_I_MOT_MAX * A2BIT_CONV) << 4;
    } else if (adc_buffer.l_tx2 > input1[0].min + 50) {
      drive_mode = 1;
      max_speed = MULTI_MODE_DRIVE_M2_MAX;
      rate = MULTI_MODE_DRIVE_M2_RATE;
      rtP_Front.n_max = rtP_Rear.n_max = MULTI_MODE_M2_N_MOT_MAX << 4;
      rtP_Front.i_max = rtP_Rear.i_max = (MULTI_MODE_M2_I_MOT_MAX * A2BIT_CONV) << 4;
    } else {
      drive_mode = 0;
      max_speed = MULTI_MODE_DRIVE_M1_MAX;
      rate = MULTI_MODE_DRIVE_M1_RATE;
      rtP_Front.n_max = rtP_Rear.n_max = MULTI_MODE_M1_N_MOT_MAX << 4;
      rtP_Front.i_max = rtP_Rear.i_max = (MULTI_MODE_M1_I_MOT_MAX * A2BIT_CONV) << 4;
    }
  ...
  while(1) {
    ...
    calcAvgSpeed();                       // Calculate average measured speed: speedAvg, speedAvgAbs
      ...
      if (enable == 0 && !rtY_Front.z_errCode && !rtY_Rear.z_errCode &&
      ...
      if (inIdx == CONTROL_ADC) {                                   // Only use use implementation below if pedals are in use (ADC input)
        ...
        if (input1[inIdx].cmd > 30) {                               // If Brake pedal (input1) is pressed, bring to 0 also the Throttle pedal (input2) to avoid "Double pedal" driving
          ...
          cruiseControl((uint8_t)rtP_Front.b_cruiseCtrlEna);         // Cruise control deactivated by Brake pedal if it was active
        }
      }
      #endif
      ...
      #else
        // ####### MIXER #######
        mixerFcn(speed << 4, steer << 4, &cmdR, &cmdF);   // This function implements the equations above
      #endif
      ...
      if (nunchuk_connected == 0) {
        cmdF = cmdF * 0.8f + (CLAMP(distanceErr + (steering*((float)MAX(ABS(distanceErr), 50)) * ROT_P), -850, 850) * -0.2f);
    ...
    // ####### SIDEBOARDS HANDLING #######
    #if defined(SIDEBOARD_SERIAL_USART2)
      sideboardSensors((uint8_t)Sideboard_F.sensors);
    #endif
    #if defined(FEEDBACK_SERIAL_USART2)
      sideboardLeds(&Sideboard_Leds_F);
    #endif
    #if defined(SIDEBOARD_SERIAL_USART3)
    ...
    front_dc_curr  = -(rtU_Front.i_DCLink * 100) / A2BIT_CONV;   // Front DC Link Current * 100
    rear_dc_curr = -(rtU_Rear.i_DCLink * 100) / A2BIT_CONV;  // Rear DC Link Current * 100
    dc_curr       = front_dc_curr + rear_dc_curr;            // Total DC Link Current * 100
    ...
    // ####### DEBUG SERIAL OUT #######
      ...
      if (main_loop_counter % 25 == 0) {    // Send data periodically every 125 ms
          ...
          process_debug();
        #else
          printf("in1:%i in2:%i cmdF:%i cmdR:%i BatADC:%i BatV:%i TempADC:%i Temp:%i \r\n",
            input1[inIdx].raw,        // 1: INPUT1
            input2[inIdx].raw,        // 2: INPUT2
            cmdF,                     // 3: output command: [-1000, 1000]
    ...
    // ####### FEEDBACK SERIAL OUT #######
      ...
      if (main_loop_counter % 2 == 0) {    // Send data periodically every 10 ms
        ...
        Feedback.speedR_meas	  = (int16_t)rtY_Rear.n_mot;
        Feedback.SpeedF_meas	  = (int16_t)rtY_Front.n_mot;
        Feedback.batVoltage	    = (int16_t)batVoltageCalib;
        Feedback.boardTemp	    = (int16_t)board_temp_deg_c;

        #if defined(FEEDBACK_SERIAL_USART2)
          if(__HAL_DMA_GET_COUNTER(huart2.hdmatx) == 0) {
            Feedback.cmdLed     = (uint16_t)Sideboard_Leds_F;
            Feedback.checksum   = (uint16_t)(Feedback.start ^ Feedback.cmd1 ^ Feedback.cmd2 ^ Feedback.speedR_meas ^ Feedback.SpeedF_meas
    ...
    if (TEMP_POWEROFF_ENABLE && board_temp_deg_c >= TEMP_POWEROFF && speedAvgAbs < 20){  // poweroff before mainboard burns OR low bat 3
      ...
      #endif
      poweroff();
    } else if (rtY_Front.z_errCode || rtY_Rear.z_errCode) {                                           // 1 beep (low pitch): Motor error, disable motors
      enable = 0;
      beepCount(1, 24, 1);
      ...
      beepCount(0, 10, 30);
    } else if (BEEPS_BACKWARD && (((cmdR < -50 || cmdF < -50) && speedAvg < 0) || MultipleTapBrake.b_multipleTap)) { // 1 beep fast (high pitch): Backward spinning motors
      beepCount(0, 5, 1);
      backwardDrive = 1;
    } else {  // do not beep
      beepCount(0, 0, 0);
    ...
    // ####### INACTIVITY TIMEOUT #######
    if (abs(cmdF) > 50 || abs(cmdR) > 50) {
      inactivity_timeout_counter = 0;
    }

    #if defined(CRUISE_CONTROL_SUPPORT) || defined(STANDSTILL_HOLD_ENABLE)
      if ((abs(rtP_Front.n_cruiseMotTgt)  > 50 && rtP_Front.b_cruiseCtrlEna) ||
          (abs(rtP_Rear.n_cruiseMotTgt) > 50 && rtP_Rear.b_cruiseCtrlEna)) {
        inactivity_timeout_counter = 0;
      }
    #endif
